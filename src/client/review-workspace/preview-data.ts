...
const examples: Example[] = [
  ...
  { projectId: "101", iid: "403", title: "重构差异解析器", branch: "refactor/diff-parser", status: "reviewing", phase: "preparing_git" },
  { projectId: "101", iid: "404", title: "修复跨文件引用分析中的边界条件，保留必要的调用方上下文", branch: "fix/cross-file-references", status: "reviewing", phase: "running_opencode" },
  { projectId: "101", iid: "405", title: "整理结构化检视结果与证据引用", branch: "feat/structured-results", status: "reviewing", phase: "saving_report" },
  { projectId: "101", iid: "406", title: "优化仓库快照清理", branch: "perf/snapshot-cleanup", status: "stopping", phase: "cleaning_up" },
  { projectId: "101", iid: "407", title: "支持中断超时的模型请求", branch: "fix/model-timeout", status: "stopped", phase: "running_opencode" },
  { projectId: "202", iid: "601", title: "完善报告加载失败时的诊断信息", branch: "fix/report-diagnostics", status: "review_failed", phase: "running_opencode" },
...
const reviewError: SafeErrorView = {
  code: "OPENCODE_FAILED", message: "检视失败。", cause: "OpenCode 进程执行失败。",
  impact: "本次不生成可处理意见，已有检视历史仍可查看。", nextStep: "检查 OpenCode 日志后重新检视。",
  technicalDetails: "OpenCode process exited with code 1.",
...
const findingTemplates: Array<Pick<StoredFinding, "severity" | "body">> = [
  {
    severity: "fatal",
...
function makeFindings(statuses: FindingStatus[], minutes: number): StoredFinding[] {
...
function makeAttempt(mr: MergeRequestSnapshot, status: AttemptStatus, suffix: string, minutes: number, findings: StoredFinding[], result?: "pass" | "findings"): AttemptView {
  ...
  return {
    ...
    sourceSha: "a1b2c3d4".repeat(5), targetSha: "b2c3d4e5".repeat(5),
...
function report(attempt: AttemptView): string {
  return [
    ...
    "## 检视结论", "", attempt.result === "pass" ? "未发现问题。" : `本次记录 ${attempt.findings.length} 项检视意见，处理结果见问题卡片。`, "",
    ...attempt.findings.flatMap(finding => [finding.body, ""]),
    "## 查证范围", "", "核对了变更代码、相关调用方与已有测试，按固定提交记录结果。", "",
    "```text", `source: ${attempt.sourceSha}`, `target: ${attempt.targetSha}`, "```", "",
  ...
  examples.forEach((example, index) => {
    ...
    if (example.status !== "unreviewed") {
      ...
      attempts.push(latest,
        ...
        makeAttempt(mr, "archived", "history-1", minutes - 120, makeFindings(["archived"], minutes - 120), "findings"));
    }
