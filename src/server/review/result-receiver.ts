...
export class ResultReceiver {
    ...
    try {
      ...
      if (tool === "reviewx_index") {
        ...
        output = { ...this.context.scope, changes: this.indexPages[page], page, totalPages: this.indexPages.length,
          ...
          requiredBase: this.indexPages[page].filter((c) => c.type === "D" || c.type === "R").map((c) => c.oldPath) };
  ...
  validate(result: ReviewSubmission) {
    if (result.completion !== "complete" || result.blockers.length || this.context.scope.changes.some((c) => c.unsupported) || [...this.required].some((r) => !this.delivered.has(r))) throw reviewError("REVIEW_INCOMPLETE", "必需材料、规则或可审范围不完整。");
  ...
  accept(messages: NativeMessage[], terminal: { idle: boolean; error: boolean; disconnected: boolean }) {
    if (!this.closed || this.signal.aborted || this.fatal || !terminal.idle || terminal.error || terminal.disconnected || !this.candidate) throw reviewError("OPENCODE_FAILED", "整体执行未正常结束或缺少有效提交。");
    const seen = new Set<string>();
    for (const m of messages) {
      if (m.info.sessionID !== this.sessionID || m.info.error) throw reviewError("OPENCODE_FAILED", "会话归属或消息终态异常。");
    ...
    if (!last?.time?.completed || last.finish !== "stop" || !last.modelID || !last.providerID) throw reviewError("OPENCODE_FAILED", "缺少正常完成与实际模型证据。");
