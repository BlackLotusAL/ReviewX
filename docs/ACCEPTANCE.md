...
- 卡片与详情抽屉使用 Kimi 风格和本地字体；键盘焦点、历史切换、响应式布局及减少动态效果可用。
- Findings 支持逐条发送、不发送和撤销；历史记录只读，报告折叠后保持缓存，Markdown 安全过滤和代码高亮有效。
- `/preview` 使用固定内存样例，展示基线准备 Git、运行 OpenCode、保存报告阶段；没有置信度或多轮核实字段，也不会提交数据变更请求。
- `/logs` 可读取当前会话日志；耗时在停止、失败和完成时固定，人工决策与发送评论不会延长检视耗时。旧数据缺少结束时间时显示占位。
- OpenCode 调用、Git 准备和检视输出契约与 a595cf4 相同。
