...
for (const width of [1675, 1230, 1024]) {
  test(`project heading sticks only inside its group at ${width}px`, async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: testInfo.outputPath("project-sticky.png") });
    ...
    await page.screenshot({ path: testInfo.outputPath("project-leaving.png") });
...
test("wrapped project headings are remeasured and newly added empty groups remain navigable", async ({ page }) => {
  ...
  await expect.poll(() => page.locator(".mr-group").first().evaluate(el => parseFloat((el as HTMLElement).style.getPropertyValue("--group-title-height"))))
    .toBeCloseTo((await firstTitle.boundingBox())!.height, 1);
  await page.getByRole("button", { name: "当前检视队列", exact: true }).click();
  await page.locator('.queue-entry[href="#mr-101-403"]').click();
  await expect.poll(async () => {
