// Frozen one-shot baseline for the real-model A/B evaluation only.
import { z } from "zod";
import { severityValues, type MergeRequestSnapshot, type Severity } from "@/src/shared/types";
...
interface ReviewerResult { findings: Array<{ severity: Severity; body: string }> }
const reviewerResultSchema = z.object({ findings: z.array(z.object({
  severity: z.enum(severityValues), body: z.string().refine(value => value.trim().length > 0 && !value.includes("\0")),
...
export class LegacyOpenCodeReviewer implements ReviewerPort {
  #command?: Promise<ResolvedCommand>;

  constructor(private readonly environment: NodeJS.ProcessEnv = process.env,
    private readonly evaluation: { model?: string; record?(stdout: string): void } = {}) {}
  ...
  async review(projectId: string, details: MergeRequestSnapshot, prepared: PreparedReview, signal: AbortSignal): Promise<ReviewerResult> {
    const result = await runProcess(await this.#resolved(), [
      ...
      "--pure",
      "--agent", "reviewx",
      ...(this.evaluation.model ? ["--model", this.evaluation.model] : []),
    ...
    this.evaluation.record?.(result.stdout);
